...
### Как работает TUI и взаимодействует с CLI

Проект построен в виде двух слоёв:

- **CLI** — основной слой, содержащий всю бизнес-логику, размещённую в `python -m hh_applicant_tool` и сабкомандах в `src/hh_applicant_tool/operations`.
- **TUI** — оболочка на Textual, которая не реализует отдельную логику, а лишь собирает параметры, формирует команду и запускает существующий CLI.

#### Внутренняя схема работы

1. Вы запускаете CLI или TUI.
2. Приложение загружает конфиг активного профиля из каталога `config` (или указанного через `--config-dir`/`--profile-id`).
3. Создаются рабочие объекты: конфиг, cookies, база SQLite, HTTP-сессия, API-клиент.
4. Затем вызывается нужная операция в виде subcommand (`apply-vacancies`, `reply-employers`, `update-resumes` и др.).
5. Часть действий выполняется через **HH API**, а часть — через веб-автоматизацию **Playwright**, если API недостаточно.
6. Результаты, логи, cookies и база сохраняются локально в директории профиля.

**Главный источник истины — CLI**, а **TUI — это лишь интерактивный frontend над этими командами**.

---

### Запуск и использование TUI

**Entrypoint** для TUI прописан в `pyproject.toml` и вызывается через:
...
```sh
poetry install
poetry run hh-applicant-tool-tui
```

Если проект установлен в окружение через pip:

```sh
hh-applicant-tool-tui
```

> Если при запуске TUI появляется сообщение об отсутствии зависимости `textual`, установите её отдельно. Она требуется обязательно.
...
2. При необходимости установите зависимости для браузерной авторизации:
...
4. В интерфейсе:
   - выберите профиль в разделе **Profiles**
   - выполняйте стандартные действия через соответствующие формы
   - для авторизации и нетиповых действий — используйте **Advanced runner**
   - отслеживайте процесс выполнения в **Live Output**

---

### Возможности TUI

В TUI реализованы готовые экраны для частых операций:
...
- **Logs** — просмотр логов
- **Live Output** — прямой вывод из выполняемой команды
- **Advanced runner** — запуск любых CLI-команд без дублирования логики в TUI

#### Как TUI запускает команды

TUI собирает команду вида:

```sh
python -m hh_applicant_tool <subcommand> ...args
```

И дополнительно:

- подставляет активный `--profile-id`
- подставляет `--config-dir` при необходимости
- для POSIX запускает интерактивные команды в **PTY-режиме**
- выводит stdout/stderr в **Live Output**
- передаёт строки во `stdin` для интерактивных сценариев

Редкие команды (`authorize`, `query`, `migrate-db`, `call-api`, `log`, любые свои параметры) лучше запускать через **Advanced runner**.
